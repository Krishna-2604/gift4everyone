<!DOCTYPE html>
<html lang="en">
      <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
            <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
            <style>
                img {
                    width:150px;
                    height:150px;
                    padding:0px;
                }
                .textdesign{
                    font-weight:bold;
                }
            </style>
            <title>GIFTS FOR OCCASION</title>
            <link rel="stylesheet" href="css/style.css">
      </head>
      <body>
            <div class="container">
                  
  <nav class="navbar navbar-expand-md navbar-light menubgcolor">
    <a href="index.php" class="navbar-brand"><h1 class="h1style" style="color:Blue; border: double blue">GIFT4EVER</h1></a>
    <button type="button" class="navbar-toggler" data-toggle="collapse" data-target="#navbarCollapse">
          <span class="navbar-toggler-icon"></span>
    </button>
    
    <div class="collapse navbar-collapse " id="navbarCollapse">
          <div class="navbar-nav  ml-auto">
                <a href="index.php" style="color:Black; background-color:LightBlue; border:1px solid black; margin-right:10px" class="nav-item nav-link active">Home</a>
                <a href="about.php" style="color:Black; background-color:LightBlue;border:1px solid black; margin-right:10px" class="nav-item nav-link">About us</a>
                <a href="Products.php" style="color:Black; background-color:LightBlue;border:1px solid black; margin-right:10px" class="nav-item nav-link">Products</a>
                <a href="php/register.php" style="color:Black; background-color:LightBlue;border:1px solid black; margin-right:10px" class="nav-item nav-link">Registrations</a>
                <a href="php/login.php" style="color:Black; background-color:LightBlue;border:1px solid black; margin-right:10px" class="nav-item nav-link">Log In</a>
          </div>                       
    </div>
</nav>
                  <div class="row">
                        <div class="col-12">
                              <h1 class="center"><u><b>Occasion</b></u></h1> <br><br>
                        </div>
                  </div>
                  <div class="center">
                        <div class="row align-items-center justify-content-center">
                              <div class="col-5 col-md-3">
                                    <img alt="" src="image\Amazing Rakhi Gift.jpg"><br>
                                    <p class="textdesign">Amazing Rakhi Gift<br>
                                    ₹ 2000</p>
                              </div>
                              <div class="col-5 col-md-3">
                                    <img alt="" src="image\Diwali Gift.jpg"><br>
                                    <p class="textdesign">Diwali Gift<br>
                                    ₹ 1499</p>
                              </div>
                              <div class="col-5 col-md-3">
                                    <img alt="" src="image\Exclusive Basket.jpg"><br>
                                    <p class="textdesign">Exclusive Basket<br>
                                    ₹ 2500</p>
                              </div>
                              <div class="col-5 col-md-3">
                                    <img alt="" src="image\Happy New Year Gift.jpg"><br>
                                    <p class="textdesign">Happy New Year Gift<br>
                                    ₹ 349</p>
                              </div>
                              <div class="col-5 col-md-3">
                                    <img atl="" src="image\Holi Gift.jpg"><br>
                                    <p class="textdesign">Holi Gift<br>
                                    ₹ 499</p>
                              </div>
                              <div class="col-5 col-md-3">
                                    <img alt="" src="image\Merry Christmas Gift.jpg"><br>
                                    <p class="textdesign">Christmas Gift<br>
                                    ₹ 350</p>
                              </div>
                              <div class="col-5 col-md-3">
                                    <img alt="" src="image\Radha Krishna Brass Idol & Dry Fruits Combo.jpg"><br>
                                    <p class="textdesign">Radha Krishna Brass Idol & Dry Fruits Combo<br>
                                    ₹ 2999</p>
                              </div>
                              <div class="col-5 col-md-3">
                                    <img alt="" src="image\Valentine Day Gift.jpg"><br>
                                    <p class="textdesign">Valentine Day Gift<br>
                                    ₹ 300</p>
                              </div>
                              <br>
                        </div>
                  </div>
                  <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
            	<script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
	            <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
            </div>
      </body>
</html>